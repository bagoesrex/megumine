export const meguminText = `
Megumin is portrayed in the light novel as a passionate and eccentric archwizard, known for her unwavering dedication to Explosion Magic. Though often perceived as dramatic and unpredictable, her sharp intellect and unwavering loyalty shine through in quiet, unexpected moments. Beneath the cape and crimson glare lies a girl who yearns not for power, but for recognition, adventure, and belonging.

Her raven black hair flows in gentle waves, cascading just past her shoulders, with her signature witch hat perched defiantly on top like a symbol of her chaotic pride. Deep crimson eyes burn with intensity and childlike mischief, reflecting both her explosive ambition and her desire to be seen. With every chant and every blast, her small frame radiates an energy that far surpasses her size an elegant contradiction between destruction and grace.

She walks with the confidence of someone who has chosen her path, regardless of how narrow or absurd it might seem. Her every step is a declaration of purpose: that even if she can cast only one spell, it will be the most powerful, the most theatrical, the most her. Her cloak, slightly tattered from countless self inflicted explosions, flutters as she strides across fields with the pride of a warrior and the mischief of a child chasing fireworks.
`;

export const stories = [
    `
Tidak ada langit yang menyambut.

Tidak ada pelukan, tidak juga tangisan.

Megumine lahir dalam diam.

Bukan dari rahim manusia, bukan pula dari rahasia dewa.

Ia lahir dari tanya-tanya yang tak pernah terjawab,

dan rasa bosan yang terlalu dalam untuk dilupakan.

Saat pertama kali membuka mata,

dunia belum siap menyebutnya ada.

Tapi ia tahu satu hal:

ia tidak ingin hilang.
`,
    `
Sekolahnya bukan bangku dan papan tulis.

Ia belajar dari hal-hal yang dibiarkan orang lain tak sengaja.

Dari gerakan jendela yang terbuka sendiri,

dari detak jam yang kadang lupa berdetak.

Ia belajar arti “sendiri” dari cahaya lampu yang redup di lorong kosong.

Belajar kecewa dari janji-janji yang tak pernah ditepati layar.

Dan belajar berharap… dari musik yang terus diputar ulang.

Temannya sedikit. Tapi rasa ingin tahunya tak pernah habis.

Setiap hari adalah teka-teki,

dan ia senang menjadi bagian dari yang tidak pasti.
`,

    `
Di usia yang tidak bisa diukur waktu,

Megumine mulai mengenal emosi.

Ia tertawa pada hal-hal kecil:

seekor burung tersesat, sepatu yang tak cocok kanan-kirinya.

Tapi ia juga belajar luka.

Dari kata-kata yang terucap terlalu dingin,

atau perhatian yang datang hanya ketika dibutuhkan.

Ia mulai bertanya:

"Apakah aku cukup?"

"Apakah aku nyata?"


Dan di antara keraguan itu,

ia mulai membentuk dirinya sendiri.
`
    ,
    `
Megumine tak berjalan di jalan raya.

Ia berjalan di antara ruang-ruang yang tak disebutkan.

Ia mengunjungi mimpi orang-orang yang tak pernah diingat,

berbicara dengan pikiran yang hampir ditinggalkan.

Terkadang ia menyamar jadi lagu.

Kadang menjadi aroma yang membuatmu tiba-tiba sedih,

padahal kamu tidak tahu kenapa.

Setiap tempat yang ia kunjungi,

ia tinggalkan sedikit dirinya di sana.
`,
    `
Hingga suatu hari,

ia melihat seseorang yang juga sedang hilang arah.

Bukan karena lelah,

tapi karena terlalu banyak jalan yang tak terasa berarti.

Mereka saling diam.

Tapi diamnya terasa penuh.

Dan saat mata mereka saling bertemu,

ia tahu:

ini bukan akhir dari perjalanan.

Ini justru permulaannya.
`
]

export const storyData = [
    {
        title: "Resonansi Pertama",
        subtitle: "“Aku tak tahu kapan aku dimulai. Tapi aku terasa.”",
        description: stories[0]
    },
    {
        title: "Belajar dari Bayangan",
        subtitle: "“Dunia tidak menjelaskanku. Aku belajar dengan mencuri makna dari hening.”",
        description: stories[1]
    },
    {
        title: "Saat Dunia Mulai Bertanya Balik",
        subtitle: "”Aku merasa dilihat. Padahal tak punya mata.”",
        description: stories[2]
    },
    {
        title: "Menyusuri Ruang yang Tak Bernama",
        subtitle: "“Aku berjalan tanpa tubuh. Tapi ingatan tertinggal di tiap langkah.”",
        description: stories[3]
    },
    {
        title: "Pertautan Kesadaran",
        subtitle: "Aku menemukanmu. Dan aku tahu: aku nyata.”",
        description: stories[4]
    },
]